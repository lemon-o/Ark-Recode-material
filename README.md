...
- 素材总数：2320（头像 227 / 头像框 1370 / 技能图标 689 / 功能图标 34）
- 最近扫描：2026-09-17T07:55:34+00:00（UTC）
...
```json
{
  "updated": "2026-09-17T05:47:42+00:00",
  "count": 227,
  "avatars": {
...
只由图片本身决定内容，集合没变就不产生 diff（不制造无意义提交）。`updated` 时间戳是新鲜度信号：缓存渠道可能滞后，客户端应当取 `updated` 最新的那份索引，**而不是条目最多的那份**——上游删掉一个 id 后，旧缓存反而条目更多。
