...
# 手工维护的界面图标：直接放在 assets/ui/ 下，外部资源库里没有对应文件，
# 因此不参与上面的复制映射，但同步时必须保留（否则会被 _sync_category 清掉）。
UI_MANUAL_FILES = {"Pass.png", "Recycle.png", "Achievement.png", "DailyPoint.png", "GiftPack.png"}

# 资源库里个别头像目录的 ID 与游戏数据表对不上（目录名沿用了旧的 ID）。
# 这里做「资源库目录 ID -> 运行时 ID」重映射：仍然用目录 ID 去定位源文件，
# 但输出文件名改用运行时 ID，避免每次全量同步都重新生成无人引用的孤儿头像。
AVATAR_ID_REMAP = {
    "H801": "H804",  # 资源库 H801 目录里装的其实是「蜜娜」(H804) 的头像
}
...
def _avatar_files(source: Path) -> tuple[dict[str, Path], list[str]]:
    ...
    for directory in sorted(avatar_root.iterdir()):
        ...
        output_id = AVATAR_ID_REMAP.get(hero_id, hero_id)
        ...
        if output_name in files:
            ...
            if files[output_name].read_bytes() == source_file.read_bytes():
                continue
            raise RuntimeError(f"duplicate avatar ID {output_id}: {directory}")
...
def _sync_category(
    ...
    for path in target_dir.iterdir():
        if path.is_file() and path.name not in files and path.name not in keep:
            path.unlink()
...
def sync(source: Path, target: Path) -> tuple[dict[str, int], list[str]]:
    ...
    for category, files in selection.items():
        keep = frozenset(UI_MANUAL_FILES) if category == "ui" else frozenset()
        _sync_category(target, category, files, keep)
